...
class Install_model extends CI_Model {
    ...
    public function use_sql_string($email, $password) {
...
//	   	$sql = $this->db->_prep_query($sql);
        ...
        $this->insertNewAdmin($email, $password);
    }

    private function insertNewAdmin($email, $password) {
        $sql = "insert into admin (email, password, level) values ('" . $email . "','" . $password . "', '1')";
        $this->db->query($sql);
    }
